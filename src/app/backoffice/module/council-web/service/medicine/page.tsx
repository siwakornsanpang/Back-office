...
type MedicineStatus = 'published' | 'draft';
...
export default function MedicinePage() {
  ...
  const getEffectiveStatus = useCallback(
    (item: MedicineItem): MedicineStatus => {
      if (item.publishedAt) {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      setItems(rawItems.map((item: any) => ({ ...item, category: item.category ?? 'medicine' })));
    } catch (err) {
      console.error(err);
      Swal.fire({
        icon: 'error',
        title: 'เกิดข้อผิดพลาด',
        text: 'ไม่สามารถโหลดข้อมูลความรู้เรื่องยาได้',
      });
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        <div>
          <h1 className={styles.title}>จัดการความรู้เรื่องยา</h1>
        ...
        <div className={styles.controls}>
          ...
          <Link href="/backoffice/module/council-web/service/medicine/create" className={`${styles.btn} ${styles.btnAdd}`}>
            <Plus size={18} /> เพิ่มบทความ
          </Link>
        </div>
      </header>
            ...
            {isLoading ? (
              ...
              currentItems.map((item) => (
                <tr key={item.id}>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <span className={styles.badge} data-category="medicine">
                      ความรู้เรื่องยา
                    </span>
                  </td>
                  <td>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <span
                      className={`${styles.badge} ${
                        getEffectiveStatus(item) === 'published' ? styles.published : styles.draft
                      }`}
                    >
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <div className={styles.actionButtons}>
                      <Link
                        href={`/backoffice/module/council-web/service/medicine/edit/${item.id}`}
      ...
      {!isLoading && totalProcessed > 0 && (
        <div className={styles.pagination}>
          <div className={styles.pageInfo}>
            ...
            {Math.min(currentPage * ITEMS_PER_PAGE, totalProcessed)} จากทั้งหมด {totalProcessed} รายการ
          </div>
          <button
