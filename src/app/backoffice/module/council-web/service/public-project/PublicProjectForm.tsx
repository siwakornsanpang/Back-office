...
export type PublicProjectStatus = 'published' | 'draft';
...
interface PublicProjectFormProps {
  initialData?: PublicProjectItem;
  mode: 'create' | 'edit';
}

const API_URL = `${process.env.NEXT_PUBLIC_API_URL}/public-project`;

export default function PublicProjectForm({ initialData, mode }: PublicProjectFormProps) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const [title, setTitle] = useState(initialData?.title || '');
  const [content, setContent] = useState(initialData?.content || '');
  const [status, setStatus] = useState<PublicProjectStatus>(initialData?.status || 'draft');
  ...
  const handleConfirmCrop = async () => {
    ...
    try {
      const croppedFile = await getCroppedImg(imageToCrop, croppedAreaPixels, `public-project-thumb-${Date.now()}.jpg`);
      if (!croppedFile) throw new Error('Crop failed');

      const formData = new FormData();
      formData.append('file', croppedFile);

      const res = await authFetch(`${process.env.NEXT_PUBLIC_API_URL}/public-project/upload-image`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/backoffice/module/council-web/service/public-project');
      router.refresh();
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <form onSubmit={handleSave} className={styles.container}>
      <header className={styles.header}>
        <div>
          <h1 className={styles.title}>{mode === 'create' ? 'เพิ่มโครงการของประชาชน' : 'แก้ไขโครงการของประชาชน'}</h1>
          <p className={styles.breadcrumb}>
            โครงการของประชาชน /{' '}
            <span className="text-blue-600 font-medium">
              {mode === 'create' ? 'เพิ่มโครงการของประชาชน' : 'แก้ไขโครงการของประชาชน'}
            </span>
          </p>
        </div>
      ...
      <div className={styles.layoutGrid}>
        <div className={styles.mainColumn}>
          <div className={styles.card}>
            ...
            <div className={styles.formGroup}>
              <input
                ...
                required
                placeholder="ระบุหัวข้อโครงการ..."
              />
            </div>
          </div>

          <div className={styles.card}>
            <h3 className={styles.cardTitle}>รายละเอียดโดยย่อ</h3>
            <div className={styles.formGroup}>
              <textarea
                className={styles.textarea}
                value={excerpt}
                onChange={(e) => setExcerpt(e.target.value)}
                placeholder="ระบุรายละเอียดโดยย่อ..."
                rows={3}
              />
            </div>
          </div>
          ...
          <div className={`${styles.card} ${styles.cardCompact}`}>
            ...
            <Editor value={content} onChange={setContent} placeholder="พิมพ์รายละเอียด หรือกดปุ่มรูปภาพเพื่ออัปโหลด..." />
          </div>
        </div>
          ...
          <div className={styles.card}>
            ...
            <div className={styles.thumbnailContainer}>
              <div
                ...
                onClick={() => document.getElementById('public-project-thumb-input')?.click()}
              >
                {thumbnailUrl ? (
