...
export default function CouncilCategoryPage() {
  ...
  const category = (Array.isArray(params.category) ? params.category[0] : params.category) || 'elected';
  const titleTH = category === 'elected' ? 'กรรมการเลือกตั้ง (12 ท่าน)' : 'กรรมการแต่งตั้ง (12 ท่าน)';
  ...
  const handleUpdate = (order: number, field: keyof CouncilSlot, value: any) => {
    setSlots(prev => prev.map(s => s.order === order ? { ...s, [field]: value, isModified: true } : s));
  };
  ...
  const handleSave = async (slot: CouncilSlot) => {
    ...
    try {
        ...
        const res = await fetch(`${API_URL}/council/save`, { method: 'POST', body: formData });

        if (res.ok) {
            await MySwal.fire({ icon: 'success', title: 'บันทึกเรียบร้อย', timer: 1000, showConfirmButton: false });
            fetchData();
        } else {
            throw new Error();
        }
  ...
  const handleClear = async (slot: CouncilSlot) => {
    ...
    if (result.isConfirmed) {
        ...
        try {
            ...
            await MySwal.fire({ icon: 'success', title: 'ลบเรียบร้อย', timer: 1000, showConfirmButton: false });
            fetchData();
        } catch (err) {
          ...
          <div key={slot.order} className={styles.card}>
            ...
            <div className={styles.imageArea}>
                ...
                {slot.tempPreview || slot.imageUrl ? (
                    <img src={slot.tempPreview || slot.imageUrl!} alt={slot.name} className={styles.memberImage} />
                ) : (
                ...
                <label className={styles.uploadOverlay}>
                    ...
                    <input 
                        ...
                        onChange={(e) => { if(e.target.files?.[0]) handleFileChange(slot.order, e.target.files[0]); }}
                    />
                </label>
            </div>
...
<<<<<<< HEAD
            <div className={styles.infoArea}>
                <input 
                    ...
                    value={slot.name}
                    onChange={(e) => handleUpdate(slot.order, 'name', e.target.value)}
                />
                ...
                <div className="relative w-full">
                    <select 
                        ...
                        value={slot.position}
                        onChange={(e) => handleUpdate(slot.order, 'position', e.target.value)}
                    >
                ...
                <div className={styles.actions}>
                    {slot.isModified && (
                        <button className={styles.saveBtn} onClick={() => handleSave(slot)} title="บันทึก">
                            <Save size={20} />
                        </button>
                    )}
                    {(slot.name || slot.imageUrl) && (
                        <button className={styles.clearBtn} onClick={() => handleClear(slot)} title="ล้างข้อมูล">
                            <Trash2 size={20} />
                        </button>
                    )}
                </div>
